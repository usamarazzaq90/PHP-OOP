...
function db_connect() {
  ...
  echo "<script>console.log(" . json_encode("database connected") . ");</script>";
  return $conn;
}
...
function insert($data = array()) {
  ...
  // Debugging: Check why prepare failed
  if (!$stmt) {
  ...
  // Return status
  if ( $insert ) {
    // echo "conn->insert_id: $conn->insert_id";
...
function show_results($insert_id) {
  // die("Reached show result function!");
  // Connect to database
  $conn = db_connect();
